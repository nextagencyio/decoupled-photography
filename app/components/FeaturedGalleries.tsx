...
import { DrupalGallery } from '@/lib/types'
import Link from 'next/link'
import ResponsiveImage from './ResponsiveImage'
import { ArrowRight } from 'lucide-react'

export default function FeaturedGalleries() {
  const { data, loading } = useQuery(GET_FEATURED_GALLERIES)
  const items: DrupalGallery[] = data?.nodeGalleries?.nodes || []
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="font-display text-3xl md:text-4xl font-bold text-primary-900 mb-4">Featured Work</h2>
          </div>
    ...
    <section className="py-20 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between mb-12">
          <div>
            <h2 className="font-display text-3xl md:text-4xl font-bold text-primary-900 mb-2">Featured Work</h2>
            <p className="text-lg text-gray-600">Explore our latest photography projects and collections.</p>
          </div>
          <Link
            href="/gallery"
