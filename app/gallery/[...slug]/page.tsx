...
import { GET_GALLERY_BY_PATH } from '@/lib/queries'
...
interface GalleryByPathData {
  route: {
    entity: DrupalGallery
  } | null
}

async function getGallery(path: string): Promise<DrupalGallery | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<GalleryByPathData>({
      query: GET_GALLERY_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/gallery/${slug.join('/')}`
  const item = await getGallery(path)

  if (!item) {
    return { title: 'Gallery Not Found | Photography' }
  }
